...
pub fn build_composite_circuit_from_public_and_fhe_dec<P: Poly, E: Evaluable<P>>(
    ...
    let (inputs, scalar_inputs) = circuit.input(num_input, num_pub_circuit_scalar_input);
    let pub_circuit_inputs = &inputs[0..num_pub_circuit_input];
    let minus_t_bar = &inputs[num_pub_circuit_input];
    let pub_circuit_scalar_inputs = &scalar_inputs;
    let circuit_id = circuit.register_sub_circuit(public_circuit);
    let pub_circuit_outputs =
        circuit.call_sub_circuit(circuit_id, pub_circuit_inputs, pub_circuit_scalar_inputs);
...
mod tests {
    ...
    fn test_build_composite_circuit_from_public_and_fhe_dec() {
        ...
        let eval_input = DCRTPoly::const_one(&params);
        ...
        let inputs = vec![eval_input.clone(), minus_t_bar.clone()];
        ...
        for i in 0..log_q {
            let a_bit_scalared = a_decomposed[i].clone() * eval_input.clone();
            let b_bit_scalared = b_decomposed[i].clone() * eval_input.clone();
            let expected_output = a_bit_scalared * minus_t_bar.clone() + b_bit_scalared;
