...
impl<M: PolyMatrix> BggPublicKey<M> {
    pub fn to_compact_bytes(&self, bit_size: usize) -> SerializableBggPublicKey {
        SerializableBggPublicKey {
            matrix: self.matrix.to_compact_bytes(bit_size),
...
impl SerializableBggPublicKey {
    pub fn from_compact_bytes<M: PolyMatrix>(
        ...
        BggPublicKey {
            matrix: M::from_compact_bytes(params, bit_size, self.matrix),
            reveal_plaintext: self.reveal_plaintext,
        }
    }
}
...
impl<M: PolyMatrix> BggEncoding<M> {
    pub fn to_compact_bytes(&self, bit_size: usize) -> SerializableBggEncoding {
        SerializableBggEncoding {
            vector: self.vector.to_compact_bytes(bit_size),
            pubkey: self.pubkey.to_compact_bytes(bit_size),
            // todo: we don't know yet how to turn poly into bytes
            plaintext: None,
        }
    }
