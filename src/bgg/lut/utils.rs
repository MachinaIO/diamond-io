...
pub fn bgg_encodings_and_input(
    ...
    let tag_bytes = tag.to_le_bytes();
    // Create secret and plaintexts
    let s_x_l = uniform_sampler.sample_uniform(&params, d, 1, DistType::BitDist);
    ...
    plaintexts.extend(
        (0..input_size).into_iter().map(|i| DCRTPoly::const_int(params, inputs[i])).collect_vec(),
    );
    plaintexts.push(minus_t_bar);
    // Create random public keys
    let p_x_l_error =
        uniform_sampler.sample_uniform(&params, 1, m, DistType::GaussDist { sigma: p_sigma });
    let reveal_plaintexts = vec![true; plaintexts.len() + 1];
    let bgg_encoding_sampler =
        BGGEncodingSampler::new(params, &s_x_l.get_row(0), uniform_sampler, 0.0);
    let pubkeys = bgg_pubkey_sampler.sample(&params, &tag_bytes, &reveal_plaintexts);
    let encodings = bgg_encoding_sampler.sample(&params, &pubkeys, &plaintexts);
