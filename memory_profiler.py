...
def run_cargo_test():
    ...
    command = [
        "cargo", "test", "-r", "--test", "test_io_dummy_param", 
...
def main():
    ...
    parser.add_argument('--log-file', type=str, help='Path to an existing log file to analyze')
    parser.add_argument('--run-test', action='store_true', help='Run a new test and analyze the results')
    parser.add_argument('--output', choices=['console', 'file', 'both'], default='both', 
                        help='Where to output the results (console, file, or both)')
    ...
    # Either analyze an existing log file or run a new test
    if args.log_file:
        print(f"Analyzing existing log file: {args.log_file}")
        df = analyze_log_file(args.log_file)
        log_file_path = args.log_file
    elif args.run_test or not args.log_file:
        # Run the cargo test command and capture the output
